...
paye_stats <- paye_emp_stats %>% 
  ...
  mutate(d_change_feb20 = case_when(date_day <= as.Date("2020-02-01") ~ NaN,
         ...
         d_change_mom = case_when(date_day - min(date_day) < months(1) ~ NaN,
                                  date_day - min(date_day) > months(1) ~ 
                                    (measure_value - lag(measure_value, n = 1))),
         p_change_mom = case_when(date_day - min(date_day) < months(1) ~ NaN,
                                  date_day - min(date_day) > months(1) ~ 
                                    (measure_value - lag(measure_value, n = 1))/lag(measure_value, n = 1)),
         d_change_yoy = case_when(date_day - min(date_day) < years(1) ~ NaN,
                                  date_day - min(date_day) > years(1) ~ 
                                    (measure_value - lag(measure_value, n = 12))),
         p_change_yoy = case_when(date_day - min(date_day) < years(1) ~ NaN,
                                  date_day - min(date_day) > years(1) ~ 
...
#### PAYE UK Regions ----
paye_nuts1_emp_stats <- readxl::read_excel(path = paste0(OTHERDATA,paye_data_list["overall"]), sheet = "7. Employees (NUTS1)", skip = 6) %>% 
  mutate( date_day = dmy(paste0("01",Date)),
          measure_name="emps")  %>% 
  select(-Date)
  

paye_nuts1_pay_stats <- readxl::read_excel(path = paste0(OTHERDATA,paye_data_list["overall"]), sheet = "8. Median pay (NUTS1)", skip = 6) %>% 
  mutate( date_day = dmy(paste0("01",Date)),
          measure_name="median_pay")  %>% 
  select(-Date)

paye_nuts1_stats <- paye_nuts1_emp_stats %>% 
  rbind(paye_nuts1_pay_stats)%>% 
  pivot_longer(cols=-c(date_day,measure_name),names_to = "geography_name", values_to = "measure_value") %>% 
  group_by(geography_name,measure_name) %>% 
  mutate( d_change_feb20 = case_when(date_day <= as.Date("2020-02-01") ~ NaN,
                                     date_day > as.Date("2020-02-01") ~ measure_value - measure_value[date_day == "2020-02-01"]),
          p_change_feb20 = case_when(date_day <= as.Date("2020-02-01") ~ NaN,
...
remove(paye_nuts1_emp_stats,paye_nuts1_pay_stats)
