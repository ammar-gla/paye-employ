...
wfj_stats <- wfj_stats_raw %>% 
  ...
  mutate( 
    p_change_dec19 = case_when(date_day <= as.Date("2019-12-01") | measures_name == "Percent" ~ NaN,
                                         date_day > as.Date("2019-12-01") ~ 100*(obs_value - obs_value[ date_day == "2019-12-01"])/(obs_value[ date_day == "2019-12-01"])),
    d_change_dec19 = case_when(date_day <= as.Date("2019-12-01") | measures_name == "Percent" ~ NaN,
                                    date_day > as.Date("2019-12-01") ~ (obs_value - obs_value[ date_day == "2019-12-01"])),
    p_change_yoy = case_when(measures_name == "Percent" ~ NaN,
                               TRUE ~ 100*(obs_value - lag(obs_value, n = 4))/(lag(obs_value, n = 4))),
    d_change_yoy = case_when(measures_name == "Percent" ~ NaN,
                               TRUE ~ (obs_value - lag(obs_value, n = 4))),
    p_change_qoq = case_when(measures_name == "Percent" ~ NaN,
                                   TRUE ~ 100*(obs_value - lag(obs_value, n = 1))/(lag(obs_value, n = 1))),
    d_change_qoq = case_when(measures_name == "Percent" ~ NaN,
  ...
  mutate(p_change_dec19_within_date_rank = case_when(industry_name == "Total" ~ NA_integer_, 
                                                      TRUE  ~ dense_rank(desc(p_change_dec19))), 
         p_change_qoq_within_date_rank = case_when(industry_name == "Total" ~ NA_integer_, 
                                                      TRUE  ~ dense_rank(desc(p_change_qoq))),
         p_change_yoy_within_date_rank = case_when(industry_name == "Total" ~ NA_integer_, 
                                                   TRUE  ~ dense_rank(desc(p_change_yoy)))) %>% 
