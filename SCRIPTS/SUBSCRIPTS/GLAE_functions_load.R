...
GLAbarchart <- function( data_set = lfs_regions_charting,
                         ...
                         caption = NULL,
                         chart_name = NULL,
                         bar_stat_type="pct rate") 
{
  
  checkmate::assert_choice(bar_stat_type, choices = c("pct rate","pct change","level","money"))
  ...
  if (bar_stat_type=="pct rate") {
    y_var_transform <- 100 # to turn into percentage decimal
    y_var_label <-  function(x) (paste0("Rate: ",perc_form(x),"%")) 
    label_form <- quote(percent_format(accuracy=1))
    
  }  else if (bar_stat_type=="pct change") {
    y_var_transform <- 1
    y_var_label <- function(x) (paste0("Change: ",perc_form(x*100),"%"))
    label_form <- quote(percent_format(accuracy=1))
  }  else if (bar_stat_type=="money") {
    y_var_transform <- 1
    y_var_label <- function(x) (paste0("Level: £",value_form(x,s=3)))
    label_form <- quote(comma_format(prefix = "£"))
    
  }  else {
    y_var_transform <- 1
    y_var_label <- function(x) (paste0("Level: ",value_form(x)))
    label_form <- quote(comma_format())
  }
  ...
  lfs_barchart <- data_set %>% 
    ...
    ggplot(mapping = aes(x = reorder({{x_var}},chart_ranking), 
                         y = {{y_var}}/y_var_transform, # Divide to fit into percentage axis
                         fill=geography_name,
    ...
    theme_set(theme_gla(gla_theme = "default")) +
    scale_y_continuous(labels = eval(label_form),
                       limits=y_limits) +
...
map_prep <- function(dataset,
  ...
  # Add tooltip variable to dataset
  dataset_adj <- dataset %>% 
    mutate(tooltip= paste0("Change: ", perc_form(!!sym(dt_var)),"%", "\n"))
  
  
  # Create list of outputs to use in mapping functions
  map_list <- list(dataset_adj=dataset_adj,bin_cuts=bin_cuts,bin_ints=bin_ints,map_palette=map_palette,dataset_bins=dataset_bins)
                               ...
                               title)
{
  ...
  dataset_adj <- map_list$dataset_adj
  map_bins <- map_list$bin_cuts
  palette <- map_list$map_palette
  
  
  # Create the palette
  pal_func <- colorBin(palette, domain = dataset_adj[[dt_var]], bins = map_bins)
  
  # Make tool tip
  labels <- sprintf("<strong>%s</strong><br/>%s",dataset_adj$geography_name, dataset_adj$tooltip) %>%
    lapply(htmltools::HTML)
  
  # Make map
  map_output1 <- leaflet(dataset_adj) %>%
    addPolygons(fillColor = ~pal_func(dataset_adj[[dt_var]]),
                weight = 1,
                opacity = 1,
                color = "white",
                fillOpacity = 0.8,
...
# Download LM data from Nomis
lfs_conf_download <- function(release_date = NULL, #should be formatted as 'yy_mm_dd' 
                             force_download=FALSE) { # In case new datasets are needed
  ...
  release_date_form <- format(release_date,"%y_%m_%d")
  url_date_form <- tolower(format(release_date,"%b%Y"))
  
  ## URL on ONS website
  conf_online_path <- "https://www.ons.gov.uk/file?uri=/employmentandlabourmarket/peopleinwork/employmentandemployeetypes/datasets/labourforcesurveysamplingvariabilitya11/current/"
  
  
  download_path <- paste0(conf_online_path,"a11",url_date_form,".xls")
  
  # Check if most recent data is already downloaded. If not, download it now
  dta_name_file <- paste0(OTHERDATA,release_date_form,"_lfs_conf",".xls")
  
  
    # If data does not already exists, or we want to force it to re-download
    
    if (file.exists(paste0(dta_name_file)) & force_download==FALSE) {
      # do nothing
    }
    else {
      # Check if link exists
      stopifnot("URL path for confidence level data does not exist"=urlFileExist(download_path)==TRUE)
      
      # Download
      download.file(url = download_path,
                    destfile = paste0(dta_name_file),
                    mode = "wb")
    }
  
  return(dta_name_file)
}
