...
cc_changes_stats <- cc_stats_detailed_raw %>% 
  ...
  mutate(d_change_mar20 = case_when(date_day <= as.Date("2020-03-01") ~ NaN,
                                          date_day > as.Date("2020-03-01") ~ measure_value - measure_value[ date_day == "2020-03-01"]),
         p_change_mar20 = case_when(date_day <= as.Date("2020-03-01") ~ NaN,
         ...
         p_change_yoy = case_when(date_day - min(date_day) < 365 ~ NaN,
                                           date_day - min(date_day) > 365 ~ 
                                             100*(measure_value - lag(measure_value, n = 12))/lag(measure_value, n = 12)),
         d_change_mom =   (measure_value - lag(measure_value, n = 1)),
         p_change_mom = 100*(measure_value - lag(measure_value, n = 1))/lag(measure_value, n = 1),
